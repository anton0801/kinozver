jQuery(document).ready(function ($) {
    ...
    $(".genre_tab").click(function () {
        var t = $(this).attr('data-id'),
            p = $(this).parent(),
            pp = $(this).parent().parent();
        $('.genre_tab', p).removeClass('active');
        $('.genre_wrap', pp).removeClass('active');
        $(this).addClass('active');
        $('.' + t, pp).addClass('active');
        return false;
    });
    ...
    if ($('.home-t .slider').length > 0) {
        $('.home-t .slider').owlCarousel({
            rewind: true,
            autoplay: true,
            margin: 20,
            dots: true,
    ...
    $('body').on('click', '.movie-trailer', function () {
        var meta = $(this),
            metaS = meta.data('src'),
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#trailer-box").append($('#player-trailer', data).html());
    ...
    if ($(window).width() > 992) {
        for (var e = 0,
                 s = $(".m-info"),
                 t = 0; t <= s.length; t++) 3 < e ? ($(".m-info:nth-child(" + t + ") .dtinfo").addClass("mdright"), e < 5 ? e++ : (e--, e--, e--, e--)) : ($(".m-info:nth-child(" + t + ") .dtinfo").addClass("mdleft"), e++);
    } else {
        $('.dtinfo').css('display', 'none');
    }
    ;
    var $gotop = $('#gotop');
    ...
    $('.widget-seocategory').$TextSlicer({
        height: '140',
        textExpand: 'Р Р°Р·РІРµСЂРЅСѓС‚СЊ',
        textHide: 'РЎРІРµСЂРЅСѓС‚СЊ'
    });
    $('.rating-vgs').$TenRate();
    $('.rate-full').each(function () {
        var a = $(this),
            b = parseInt(a.find('.ratefull-hide .ratingtypeplusminus').text()),
            c = parseInt(a.find('.ratefull-hide span[id]:last').text());
        if (c >= b) {
            var m = (c - b) / 2,
                p = c - m,
                ten = (Math.round((c - (c - b) / 2) / c * 100)) / 10,
                id = a.data('id');
            a.children('.ratefull-plus').append('<span class="psc">' + p + '</span>');
            a.children('.ratefull-minus').append('<span class="msc">' + m + '</span>');
        }
        ;
        a.addClass('rate-done');
    });
    $('.comment-user:not(.done)').each(function () {
        var a = $(this), b = a.closest('.comment').find('.comment-avatar img'),
            d = ["#2989F9", "#fed700", "#7ed321", "#16a085", "#FFA900", "#f60"],
            rand = Math.floor(Math.random() * d.length);
        b.attr('style', 'border-color:' + d[rand] + '');
        a.addClass('done');
    });
    $('.player-light').click(function () {
        ...
        var t = $(this);
        t.text(t.text() == 'Р’РєР»СЋС‡РёС‚СЊ СЃРІРµС‚' ? 'Р’С‹Р»СЋС‡РёС‚СЊ СЃРІРµС‚' : 'Р’РєР»СЋС‡РёС‚СЊ СЃРІРµС‚');
    });
    $(".widget-menu-slist").mCustomScrollbar({});
});
...
jQuery.fn.$TextSlicer = function (options) {
    ...
    return this.each(function () {
        var a = $(this),
            h = a.outerHeight();
        if (h > options.height) {
            a.addClass('widget-seocategory slice-masked').attr('data-height', h).height(options.height).after('<div class="slice-button"><span>' + options.textExpand + '</span></div>');
        }
        ;
